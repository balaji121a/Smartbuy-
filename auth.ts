...
// SECURITY: JWT_SECRET must be its own dedicated secret.
// Never reuse an unrelated API key (e.g. GEMINI_API_KEY) as a signing secret,
// and never fall back to a hardcoded value shipped in source control —
// anyone with the repo could forge valid login tokens.
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) {
  throw new Error(
    'Missing JWT_SECRET environment variable. Set a long, random JWT_SECRET in your .env file before starting the server (see .env.example).'
  );
}

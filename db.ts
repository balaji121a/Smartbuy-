  ...
  const users = [
    {
      ...
      // NOTE: left unset intentionally. The old placeholder value here was not a valid
      // "salt:hash" PBKDF2 string, so comparePassword() would always reject it and login
      // would silently fail. server.ts hashes the real demo password ('admin123') into
      // this field on first startup (dev-only) — see the seed-password bootstrap there.
      passwordHash: ''
    },
    {
      id: 'usr_seller1',
      ...
      address: '77 Electro Way, Tech Park, CA 94107',
      // See note above — real hash for 'seller123' gets set by server.ts on first startup.
      passwordHash: ''
    },
    {
      id: 'usr_user1',
      ...
      // See note above — real hash for 'user123' gets set by server.ts on first startup.
      passwordHash: ''
    }
  ];
...
// Strips sensitive fields (passwordHash) before any user object leaves the server.
// SECURITY: never send a raw user record (with passwordHash) in an API response.
...
export const UserModel = {
  // Internal/auth-only lookups that DO include passwordHash (needed to verify login).
  // Never return the result of these two directly in an API response.
  find: async () => {
    return db.getUsers();
  },
  ...
  // Public-safe lookup: strips passwordHash automatically.
  findById: async (id: string) => {
